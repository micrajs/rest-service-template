const MakeHelper = {
  ...
  async handler({
    ...
    try {
      ...
      // Params
      const RAW_NAME = parser.getArgument(0).value || undefined;
      ...
      const FILES = [
        ...
        [
          app(NAME.KEBAB, `${NAME.KEBAB}.register.d.ts`),
          template('module.register'),
        ],
      ];
      ...
      FILES.forEach(([path, template]) => {
        createFile(
          path,
          use('TemplateEngine').render(template, defaultVariables({ NAME })),
          FORCE,
        );
      });
    } catch (e) {
